import { ICharacter, INewsEvents } from "./types";

export const testEventsData: INewsEvents[] = [
  {
    EndDate: "2023-01-18T06:00:00",
    Link: "https://lostark.game.onstove.com/Promotion/Reservation/230104",
    RewardDate: "2023-02-15T06:00:00",
    StartDate: "2023-01-04T06:00:00",
    Thumbnail:
      "https://cdn-lostark.game.onstove.com/uploadfiles/banner/1756e03eb85243d6bd4189f932d0b63b.jpg",
    Title: "슬레이어 사전 등록 이벤트",
  },
  {
    EndDate: "2023-01-18T06:00:00",
    Link: "https://lostark.game.onstove.com/Promotion/Reservation/230104",
    RewardDate: "2023-02-15T06:00:00",
    StartDate: "2023-01-04T06:00:00",
    Thumbnail:
      "https://cdn-lostark.game.onstove.com/uploadfiles/banner/1756e03eb85243d6bd4189f932d0b63b.jpg",
    Title: "슬레이어 사전 등록 이벤트",
  },
  {
    EndDate: "2023-03-15T06:00:00",
    Link: "https://lostark.game.onstove.com/Promotion/pointshop/221221",
    RewardDate: "2023-03-22T06:00:00",
    StartDate: "2022-12-21T06:00:00",
    Thumbnail:
      "https://cdn-lostark.game.onstove.com/uploadfiles/banner/383280fdceea4a6fbb6cee56741dee95.jpg",
    Title: "프레이야 포인트 SHOP",
  },
  {
    EndDate: "2023-03-15T06:00:00",
    Link: "https://lostark.game.onstove.com/Promotion/donation/221221",
    RewardDate: null,
    StartDate: "2022-12-21T06:00:00",
    Thumbnail:
      "https://cdn-lostark.game.onstove.com/uploadfiles/banner/512a5b366254482ebc714f3d8366fd1e.jpg",
    Title: "프레이야 희망의 트리",
  },
  {
    EndDate: "2023-03-15T06:00:00",
    Link: "https://lostark.game.onstove.com/Promotion/donation/221221",
    RewardDate: null,
    StartDate: "2022-12-21T06:00:00",
    Thumbnail:
      "https://cdn-lostark.game.onstove.com/uploadfiles/banner/512a5b366254482ebc714f3d8366fd1e.jpg",
    Title: "프레이야 희망의 트리",
  },
  {
    EndDate: "2023-03-15T06:00:00",
    Link: "https://lostark.game.onstove.com/Promotion/donation/221221",
    RewardDate: null,
    StartDate: "2022-12-21T06:00:00",
    Thumbnail:
      "https://cdn-lostark.game.onstove.com/uploadfiles/banner/512a5b366254482ebc714f3d8366fd1e.jpg",
    Title: "프레이야 희망의 트리",
  },
  {
    EndDate: "2023-03-15T06:00:00",
    Link: "https://lostark.game.onstove.com/Promotion/donation/221221",
    RewardDate: null,
    StartDate: "2022-12-21T06:00:00",
    Thumbnail:
      "https://cdn-lostark.game.onstove.com/uploadfiles/banner/512a5b366254482ebc714f3d8366fd1e.jpg",
    Title: "프레이야 희망의 트리",
  },
];

export const testCharsData: ICharacter[][] = [
  [
    {
      CharacterClassName: "리퍼",
      CharacterLevel: 60,
      CharacterName: "sumut",
      ItemAvgLevel: "1,477.50",
      ItemMaxLevel: "1,477.50",
      ServerName: "루페온",
    },
    {
      CharacterClassName: "데빌헌터",
      CharacterLevel: 19,
      CharacterName: "우리가만렙까지갈수있을까",
      ItemAvgLevel: "75.18",
      ItemMaxLevel: "0.00",
      ServerName: "아브렐슈드",
    },
    {
      CharacterClassName: "전사",
      CharacterLevel: 1,
      CharacterName: "1234123423",
      ItemAvgLevel: "0.00",
      ItemMaxLevel: "0.00",
      ServerName: "니나브",
    },
    {
      CharacterClassName: "전사",
      CharacterLevel: 1,
      CharacterName: "1234243123",
      ItemAvgLevel: "0.00",
      ItemMaxLevel: "0.00",
      ServerName: "카단",
    },
  ],
  [
    {
      CharacterClassName: "데빌헌터",
      CharacterLevel: 19,
      CharacterName: "우리가만렙까지갈수있을까",
      ItemAvgLevel: "75.18",
      ItemMaxLevel: "0.00",
      ServerName: "아브렐슈드",
    },
    {
      CharacterClassName: "전사",
      CharacterLevel: 1,
      CharacterName: "1234123423",
      ItemAvgLevel: "0.00",
      ItemMaxLevel: "0.00",
      ServerName: "니나브",
    },
    {
      CharacterClassName: "전사",
      CharacterLevel: 1,
      CharacterName: "1234243123",
      ItemAvgLevel: "0.00",
      ItemMaxLevel: "0.00",
      ServerName: "카단",
    },
  ],
];
